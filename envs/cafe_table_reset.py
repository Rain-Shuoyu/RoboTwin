...
class cafe_table_reset(Base_Task):
    ...
    def load_actors(self):
        ...
        self.cup = actor("021_cup", [-0.30, -0.18, 0.741], quat=[0.5, 0.5, 0.5, 0.5])
        self.mug = actor("039_mug", [0.00, -0.18, 0.741], quat=[0.5, 0.5, 0.5, 0.5])
        self.plate = actor("003_plate", [0.30, -0.18, 0.741], quat=[0.5, 0.5, 0.5, 0.5])
        self.tissue_box = actor("023_tissue-box", [-0.42, 0.18, 0.741], is_static=True, quat=[0.5, 0.5, 0.5, 0.5])
        self.tray = actor("008_tray", [0.00, 0.12, 0.741], is_static=True, mass=0.12, quat=[0.706527, 0.706483, -0.0291356, -0.0291767])
        ...
        self.roll_paper = actor("028_roll-paper", [0.00, 0.20, 0.741], model_id=1, is_static=True, quat=[0.5, 0.5, 0.5, 0.5])
        self.breadbasket = actor("076_breadbasket", [0.34, 0.18, 0.741], is_static=True, mass=0.12)
        self.bread = actor("075_bread", [0.34, 0.18, 0.741], is_static=True, mass=0.04)
    ...
    def check_success(self):
        ...
        supplies_ready = self._in_region(self.tissue_box, [-0.42, 0.18]) and self._in_region(self.cleaner, [0.26, 0.20])
        dishes_cleared = all(self._in_region(item, tray_center, (0.14, 0.12)) for item in (self.cup, self.mug, self.plate))
        return bool(
            dishes_cleared
            and self._in_region(self.tray, tray_center, (0.14, 0.12))
